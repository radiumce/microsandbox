#!/usr/bin/env python3
"""
End-to-end MCP server integration tests for LRU eviction.

This test suite validates LRU eviction through the complete MCP server stack:
1. MCP client SDK -> MCP server -> MicrosandboxWrapper -> Microsandbox server
2. Official MCP protocol communication
...
class TestMCPLRUEvictionE2E:
    """End-to-end MCP server tests for LRU eviction using official MCP SDK."""
    ...
    @pytest.fixture
    def mcp_server_params(self, server_url, api_key):
    ...
    @pytest.fixture
    async def microsandbox_health_check(self, server_url):
    ...
    async def create_mcp_session(self, server_params):
    ...
    @pytest.mark.asyncio
    async def test_mcp_e2e_lru_eviction_basic(self, mcp_server_params, microsandbox_health_check):
        """Test basic LRU eviction through MCP server using official SDK."""
        print("\n=== MCP E2E Test: Basic LRU Eviction ===")
        ...
        async with stdio_client(mcp_server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                print("\n📝 Creating sessions through MCP SDK...")
                session_results = []
...
""",
                ...
                print(f"  ✅ Created {len(session_results)} sessions through MCP SDK")
        
                # Step 4: Access some sessions to update LRU order
                print("\n🔄 Updating LRU order through MCP SDK...")
...
""",
                ...
                print(f"  ✅ LRU eviction triggered through MCP SDK")
                
                print("\n🎉 MCP E2E Basic LRU Eviction Test PASSED!")
    
    @pytest.mark.asyncio
    async def test_mcp_e2e_resource_limits(self, mcp_server_params, microsandbox_health_check):
        """Test resource limit enforcement through MCP SDK."""
        print("\n=== MCP E2E Test: Resource Limits ===")
        ...
        async with stdio_client(mcp_server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                for i in range(3):  # Max sessions = 3
                    ...
                    print(f"  Created session {i+1} through MCP SDK")
                ...
                print("\n🎉 MCP E2E Resource Limits Test PASSED!")
    
    @pytest.mark.asyncio
    async def test_mcp_e2e_error_handling(self, mcp_server_params, microsandbox_health_check):
        """Test error handling in MCP LRU eviction."""
        print("\n=== MCP E2E Test: Error Handling ===")
        ...
        async with stdio_client(mcp_server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                print("\n❌ Testing invalid code through MCP SDK...")
                ...
                print("\n❌ Testing invalid parameters through MCP SDK...")
                ...
                print("\n🎉 MCP E2E Error Handling Test PASSED!")
    
    @pytest.mark.asyncio
    async def test_mcp_e2e_session_reuse(self, mcp_server_params, microsandbox_health_check):
        """Test session reuse through MCP SDK."""
        print("\n=== MCP E2E Test: Session Reuse ===")
        ...
        async with stdio_client(mcp_server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                print("\n🎉 MCP E2E Session Reuse Test PASSED!")


async def run_mcp_e2e_tests():
    """Run MCP end-to-end tests manually using official SDK."""
    print("🚀 Running MCP End-to-End LRU Eviction Tests")
    print("=" * 70)
    ...
    try:
        ...
        test_instance = TestMCPLRUEvictionE2E()
        
        # Run tests
        await test_instance.test_mcp_e2e_lru_eviction_basic(server_params, None)
        await test_instance.test_mcp_e2e_resource_limits(server_params, None)
        await test_instance.test_mcp_e2e_error_handling(server_params, None)
        await test_instance.test_mcp_e2e_session_reuse(server_params, None)
        
        print("\n" + "=" * 70)
        print("🎉 All MCP End-to-End LRU Eviction Tests PASSED!")
        print("=" * 70)
        return True
        
    except Exception as e:
        print(f"\n❌ MCP E2E Test failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
if __name__ == "__main__":
    success = asyncio.run(run_mcp_e2e_tests())
    if not success:
        exit(1)